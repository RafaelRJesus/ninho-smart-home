...
function App({user,home,onLogout}) {
  ...
  const realtime = useRealtime(receiveEvent);
  ...
  return <div className="app">
    ...
    <main>
      ...
      {view === 'routines' && <AutomationCenter devices={devices} notify={notify}/>} 
    ...
    {adding && <AddDevice close={()=>setAdding(false)} saved={d=>{setDevices(x=>[...x,d]);setAdding(false)}} rooms={rooms}/>} 
    {managingRooms && <RoomManager rooms={rooms} changed={setRooms} close={()=>setManagingRooms(false)}/>} 
  </div>
}
...
function AddDevice({close,saved,rooms}) { const [form,setForm]=useState({name:'',room:rooms[0]?.name||'Sala',type:'light',x:50,y:50}); const [error,setError]=useState(''); async function submit(e){e.preventDefault();const r=await fetch(`${API}/devices`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)});const data=await r.json();if(!r.ok)return setError(data.error);saved(data)} return <div className="overlay"><form className="panel" onSubmit={submit}><button type="button" className="close" onClick={close}><X/></button><h2>Adicionar ponto</h2><p>Cadastre um aparelho na sua planta.</p>{error&&<div className="form-error">{error}</div>}<label>Nome<input required value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Ex: Luz da varanda"/></label><label>Ambiente<select value={form.room} onChange={e=>setForm({...form,room:e.target.value})}>{rooms.map(room=><option key={room.id}>{room.name}</option>)}</select></label><label>Tipo<select value={form.type} onChange={e=>setForm({...form,type:e.target.value})}><option value="light">Iluminação</option><option value="ac">Ar-condicionado</option><option value="tv">Televisão</option><option value="plug">Tomada</option></select></label><button className="primary" type="submit">Adicionar à planta</button></form></div> }

function RoomManager({rooms,changed,close}) {
  ...
  async function add(e){e.preventDefault();const r=await fetch(`${API}/rooms`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})});const data=await r.json();if(!r.ok)return setError(data.error);changed([...rooms,data]);setName('');setError('')}
  async function rename(room){const clean=editName.trim();if(!clean)return;const r=await fetch(`${API}/rooms/${room.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:clean})});const data=await r.json();if(!r.ok)return setError(data.error);changed(rooms.map(item=>item.id===room.id?data:item));setEditing(null);setError('')}
  async function reorder(index,direction){const next=[...rooms];const target=index+direction;if(target<0||target>=next.length)return;[next[index],next[target]]=[next[target],next[index]];const r=await fetch(`${API}/rooms/order`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids:next.map(room=>room.id)})});const data=await r.json();if(r.ok)changed(data);else setError(data.error)}
  async function remove(room){if(!window.confirm(`Excluir o cômodo “${room.name}”?`))return;const r=await fetch(`${API}/rooms/${room.id}`,{method:'DELETE'});if(!r.ok){const data=await r.json();return setError(data.error)}changed(rooms.filter(item=>item.id!==room.id));setError('')}
...
function AuthScreen({authenticated}){const [registering,setRegistering]=useState(false);const [form,setForm]=useState({displayName:'',email:'',password:''});const [error,setError]=useState('');const [busy,setBusy]=useState(false);async function submit(e){e.preventDefault();setBusy(true);setError('');try{const r=await fetch(`/api/v1/auth/${registering?'register':'login'}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)});const data=await r.json();if(!r.ok)throw new Error(data.message);await authenticated(data.user)}catch(err){setError(err.message)}finally{setBusy(false)}}return <main className="auth-page"><section className="auth-card"><div className="auth-brand"><span><Home/></span><b>Ninho</b></div><div><small>CASA INTELIGENTE, COM PRIVACIDADE</small><h1>{registering?'Crie sua casa digital':'Bem-vindo de volta'}</h1><p>{registering?'Sua conta protege dispositivos, rotinas e credenciais.':'Entre para controlar sua casa com segurança.'}</p></div><form onSubmit={submit}>{registering&&<label>Seu nome<input required value={form.displayName} onChange={e=>setForm({...form,displayName:e.target.value})}/></label>}<label>E-mail<input type="email" required value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></label><label>Senha<input type="password" minLength="10" required value={form.password} onChange={e=>setForm({...form,password:e.target.value})}/></label>{error&&<div className="form-error">{error}</div>}<button className="primary" disabled={busy}>{busy?'Aguarde...':registering?'Criar conta segura':'Entrar'}</button></form><button className="auth-switch" onClick={()=>{setRegistering(!registering);setError('')}}>{registering?'Já tenho uma conta':'Criar minha conta'}</button><div className="auth-security"><LockKeyhole/><span>Sessão HttpOnly · secrets criptografados · TLS obrigatório</span></div></section></main>}
