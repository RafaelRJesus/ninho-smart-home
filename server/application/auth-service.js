...
export class AuthService {
  ...
  async refresh(token){const payload=this.tokens.verify(token,'refresh');if(this.identity.isRevoked(payload.jti))throw new AppError('INVALID_SESSION','Sessão revogada.',401);this.identity.revoke(payload.jti);const user=await this.identity.findUser(payload.sub);if(!user)throw new AppError('INVALID_SESSION','Sessão inválida.',401);return this.issue(user);}
  logout(refreshToken){if(!refreshToken)return;try{const payload=this.tokens.verify(refreshToken,'refresh');this.identity.revoke(payload.jti)}catch{}}
}
