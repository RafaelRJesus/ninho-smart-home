...
export async function connectPostgres(url=process.env.DATABASE_URL){
  ...
  for(const name of migrations){
    ...
    const sql=await fs.readFile(path.join(migrationDir,name),'utf8');const client=await pool.connect();
    try{await client.query(sql);await client.query('INSERT INTO schema_migrations(name) VALUES($1)',[name]);}
