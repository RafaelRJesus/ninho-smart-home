...
try{
  ...
  const homeResponse=await fetch(`${base}/api/v1/homes`,{method:'POST',headers:{'Content-Type':'application/json',Cookie:access},body:JSON.stringify({name:'Casa Stress'})});if(!homeResponse.ok)throw new Error(`Criação da casa de stress falhou: ${await homeResponse.text()}`);const home=await homeResponse.json();
  await load('health-under-load','/api/health/live');
  const protectedResult=await load('authenticated-api-with-load-shedding',`/api/v1/homes/${home.id}/devices`,{headers:{Cookie:access},accepted:[200,429]});if(!protectedResult.statuses[200])throw new Error('API não respondeu nenhuma solicitação com sucesso.');
